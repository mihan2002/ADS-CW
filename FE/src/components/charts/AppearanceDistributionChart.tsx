...
export function AppearanceDistributionChart({ data }: Props) {
    ...
    <Card padding="md">
      <div ref={chartRef}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-semibold text-surface-100">Appearance Count Distribution</h3>
            <p className="text-xs text-surface-500 mt-0.5">How often alumni are featured</p>
          </div>
          <div className="flex gap-1">
            <button onClick={() => exportToCSV(data, 'appearance_distribution')}
            ...
            <button onClick={() => chartRef.current && downloadChartAsImage(chartRef.current, 'appearance_distribution')}
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            <Bar dataKey="count" name="Alumni Count" fill="#f472b6" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </Card>
